...
from sar_optimal.optimal_runner import CentralizedOptimalRunner, ParallelCentralizedOptimalRunner
...
def main():
    ...
    parser.add_argument("--parallel", action="store_true", help="Use parallel centralized runner")
    ...
    if args.parallel:
        ...
        # Usually num_workers defaults to multiprocessing.cpu_count() or a sensible default, we'll use 4
        runner = ParallelCentralizedOptimalRunner(verbose=True, num_workers=4)
        try:
            results = runner.run(uavs, tasks)
        finally:
            # Always make sure to cleanup the pool
